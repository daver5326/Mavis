  ...
  async patchColdStartDoc(sessionSummary) {
    try {
      ...
      const response = await fetch('/api/chat', {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          system: `You are the Mavis Foreman. You have the current cold start document and a session summary. Update ONLY these three sections based on the session: CURRENT SYSTEM STATE, BUILD LOG, and OPEN QUESTIONS. Do not change any other sections. Return the complete updated document as plain text. No markdown formatting. No preamble.`,
          messages: [{
            role: 'user',
