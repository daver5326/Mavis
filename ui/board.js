...
function renderBoard(cards) {
  ...
  const insightPlaceholder = `<div id="insight-card" class="board-card insight-card" style="display:none;"></div>`;

  const cardHTML = cards.map((card, i) => `
    <div class="board-card" onclick="openCardDetail(${i})">
      <div class="card-zone">${card.zone}</div>
...
function injectInsight(text) {
  ...
  if (!card) return;
  card.innerHTML = `<div class="card-zone">Mavis</div><div class="card-title">Insight</div><div class="card-body">${text}</div>`;
