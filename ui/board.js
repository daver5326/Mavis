...
function clearInsightCache(threadId) {
  if (threadId) delete _insightCache[threadId];
  else Object.keys(_insightCache).forEach(k => delete _insightCache[k]);
}
...
function buildBoardFromThread(thread, ideas) {
  ...
  if (thread['Goal']) {
    cards.push({
      zone: 'Goal', color: 'green', title: 'Goal',
      body: thread['Goal'],
  ...
  if (thread['Next step']) {
    cards.push({
      zone: 'Next', color: 'blue', title: 'Next Step',
  ...
  if (thread['Decisions made']) {
    cards.push({
      zone: 'Decisions', color: 'amber', title: 'Decisions Made',
  ...
  if (thread['Open question']) {
    cards.push({
      zone: 'Questions', color: 'red', title: 'Open Questions',
  ...
  if (thread['Note']) {
    cards.push({
      zone: 'Notes', color: 'amber', title: 'Notes',
      body: thread['Note'],
  ...
  if (ideas && ideas.length > 0) {
    cards.push({
      zone: 'Ideas', color: 'blue', title: 'Banked Ideas',
  ...
  if (thread['Current progress']) {
    cards.push({
      zone: 'Progress', color: 'green', title: 'Recent Progress',
      body: thread['Current progress'].slice(-600),
...
function renderBoard(cards) {
  ...
  const cardHTML = cards.map((card, i) => `
    <div class="card ${card.color}" data-card-index="${i}">
  ...
  container.querySelectorAll('.card[data-card-index]').forEach(el => {
    el.addEventListener('click', () => openCardDetail(parseInt(el.dataset.cardIndex)));
  });
}
...
async function generateInsightCard(thread, ideas) {
  if (_insightCache[thread.id]) return _insightCache[thread.id];
...
Ideas: ${ideas && ideas.length > 0 ? ideas.map(i => i.idea_text).join(', ') : 'none'}`;
    ...
    if (data.content && data.content[0]) {
      ...
      _insightCache[thread.id] = insight;
      return insight;
    }
    return null;
  } catch(e) { return null; }
