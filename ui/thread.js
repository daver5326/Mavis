...
async function triageReactivate(id) {
  ...
  const updated = { ...thread, 'Current progress': newProgress };
  setTimeout(() => loadThreads(), 1000);
  await openThreadDirect(updated);
  addMessage('assistant', `Back on ${updated['Thread name']}. Next step was: ${updated['Next step'] || 'not set'} — still the right move?`);
}

async function triageReview(id) {
  const thread = await fetchThread(id);
  if (!thread) return;
  await openThreadDirect(thread);
  addMessage('assistant', `Reviewing ${thread['Thread name']}. Take a look at the board — what do you want to do with it?`);
}
...
async function openThreadDirect(thread) {
  ...
  const ideas = await fetchIdeas(thread.id);
  const board = buildBoardFromThread(currentThread, ideas);
  renderBoard(board);
  ...
  const allThreads = await fetchAllThreads();
  const otherThreads = allThreads.filter(t => t.id !== thread.id && t['Status'] === 'Active');
  systemContext = buildThreadContext(currentThread, ideas, otherThreads);
}
